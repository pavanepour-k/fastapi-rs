...
use crate::core::routing::APIRoute;
...
impl FastApiRoute {
    ...
    pub fn new(path: String, methods: Vec<String>, name: Option<String>) -> PyResult<Self> {
        let mut route = APIRoute::new(path.clone(), methods.clone());
        if let Some(n) = name.clone() {
            route = route.with_name(n);
        }
    ...
    pub fn matches(&self, path: &str, method: &str) -> bool {
        self.inner.matches(path, method).is_some()
    }

    pub fn extract_params(&self, path: &str) -> Option<HashMap<String, String>> {
        self.inner.matches(path, &self.methods[0])
...
impl FastApiRoute {
    pub fn from(route: APIRoute) -> Self {
        FastApiRoute {
...
impl RequestData {
    ...
    fn __repr__(&self) -> String {
        format!(
            ...
            self.method, self.path, self.content_type, self.body_size()
        )
    }
}
